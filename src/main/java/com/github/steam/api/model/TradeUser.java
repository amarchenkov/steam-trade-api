...
public class TradeUser {
    ...
    public String fetch(String url, String method, List<NameValuePair> data, boolean ajax) throws IOException {
        ...
        Scanner s = new Scanner(response.getEntity().getContent(), "UTF-8").useDelimiter("\\A");
    ...
    public Trade newTrade(SteamID partner) throws Exception {
        return new Trade(this, 0, partner);
    }

    /**
     * Log into the specified account
     * @param username Имя пользователя в Steam Community
     * @param password Пароль от Steam Community
     * @return Флаг успешности аутентификации
     * @throws IOException
     */
        ...
        do {
            ...
            if (captcha) {
                ...
                if (Desktop.isDesktopSupported()) {
                    ...
                    LOG.info("https://steamcommunity.com/public/captcha.php?gid=" + loginJson.captcha_gid);
                }
...
//
//        ArrayList<CEconTradeOffer> offers = new ArrayList<>();
//
...
//            CEconTradeOffer tradeOffer = new CEconTradeOffer(id, active, this);
//            offers.add(tradeOffer);
//        }
//
//        return offers.toArray(new CEconTradeOffer[offers.size()]);
    }
}